// ✅ FlyerPreview.js — fixed title issue and clarified inputs
...
const FlyerPreview = ({ listing, onClose }) => {
  ...
  useEffect(() => {
    if (listing.coverImage) {
      fetch(`http://localhost:5000${listing.coverImage}`)
        .then((res) => res.blob())
        .then((blob) => {
  ...
  const handleDownload = async () => {
    ...
    const canvas = await html2canvas(flyer, {
      ...
      scale: 2, // High-res without manual transform
      logging: false,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-auto bg-black bg-opacity-70">
      <div className="bg-white rounded-lg p-4 max-w-[95vw] overflow-auto">
        <div
          ref={flyerRef}
          className="relative w-[1080px] h-[1080px] bg-white overflow-hidden"
        >
          {/* Cover Image */}
          {base64Cover && (
            <img
